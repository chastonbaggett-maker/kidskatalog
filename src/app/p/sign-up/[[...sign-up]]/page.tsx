...
import { ParentClerkSignUp } from "@/components/parent/ParentClerkSignUp";
...
export default async function ParentSignUpPage({ searchParams }: Props) {
  ...
  return (
    <div className="shelf-page star-field flex min-h-0 flex-1 flex-col overflow-hidden">
      <ShelfHeader
        title="Parent Mode"
        subtitle="Sign up to save wish lists"
        backHref="/p"
        logoHref="/p"
      ...
      <div className="page-scroll star-field min-h-0 flex-1 px-4 py-4 scroll-pad-bottom">
        <div className="mx-auto w-full max-w-md">
          {isClerkConfigured() ? (
            <ParentClerkSignUp returnTo={returnTo} />
          ) : (
            <ParentAuthForm mode="signup" returnTo={returnTo} />
          )}
        </div>
      </div>
    </div>
